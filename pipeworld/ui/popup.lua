...
--
-- Missing:
--  * Need hover to show 'description'
--  * Right-column short right-align short-cut / symbol list (render_text limitation)
--  * Single-line separator (render_text limitation)
--
-- Notes:
...
-- raw just merges table into full one
		if v.raw then
			for _, v in ipairs(v.raw) do
				table.insert(text_list, v)
			end
-- otherwise we construct the format string
		else
...
local function drop(ctx)
	if ctx.animation_out > 0 then
		blend_image(ctx.anchor, 0, ctx.animation_out, ctx.interp)
...
--  interp        - animation interpolation function
--  border_attach - function(ctx : tbl, src : vid)
--                  end
...
--
local function replace(popup, menu)
-- build string table
	if #menu == 0 then
		hide_image(popup.anchor)
		return
	end

	local text_list, symbol_list = menu_to_fmtstr(menu, popup.options)
	if not text_list then
		return
	end

	show_image(popup.anchor)

-- render failure is bad font or out of VIDs, drop the group
	local _, lh, _, _, ascent = render_text(popup.text, text_list)
	popup.line_pos = lh
	popup.cursor_h = ascent

	local props = image_storage_properties(popup.text)
	popup.max_w = props.width
	popup.max_h = props.height

	resize_image(popup.anchor, popup.max_w, popup.max_h)
	if popup.options.border_attach then
		popup.options.border_attach(popup, popup.anchor)
	end
...
-- out of VIDs?
	...
	res.interp = options.interp and options.interp or INTERP_SMOOTHSTEP

	blend_image(res.anchor, 1.0, res.animation_in, res.interp)
...
-- disable mouse handling, we do that for the anchor surface
	...
	res.max_w = props.width
	res.max_h = props.height
...
-- based on dy from last time, but cursors can be jumpy.
	...
	if not options.mouse_block then
		mouse_addlistener(mh, {"motion", "click", "button"})
	end

	if (not options.cursor_at) then
		res.cursor = color_surface(64, 64, 255, 255, 255);
	else
		res.cursor = options.cursor
	end

	if valid_vid(res.cursor) then
		link_image(res.cursor, res.anchor)
		blend_image(res.cursor, 0.5)
		image_inherit_order(res.cursor, true)
		order_image(res.cursor, 2)
		image_mask_set(res.cursor, MASK_UNPICKABLE)
	end
